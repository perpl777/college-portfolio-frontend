        ...
        <div className='p-11 flex flex-col justify-between gap-8 max-sm:p-7'>
            ...
            <div className='flex justify-between items-end max-sm:flex-col max-sm:items-start max-sm:gap-7'>
                <div className='w-7/12 max-sm:w-10/12'>
                    { description &&
                        <p className='text-sm opacity-70 pb-14'>{description}</p>
                    }

                    {link &&
                        <Link href={link} target='_blank' className='cursor-pointer text-sm text-slate-400 hover:text-sky-800'>Ссылка: {link}</Link>
                    }
                </div>

                { file &&
                    <Link href={file} target='_blank'><Image src={PdfIcon} alt="Ссылка на PDF" width={30}/></Link>
                }
            </div>
