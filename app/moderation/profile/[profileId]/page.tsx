...
export default function Profile({ params: {profileId}}: Props) {
    ...
    <>
        { user && userRole?.role.name === "Moderator" &&
            <div>
                <Header />
                <div className='px-11 pb-12 pt-16 max-sm:px-6 max-sm:pb-8 max-sm:pt-10'>
                    <Link href={`#${profileId}`} onClick={() => window.history.back()}>
                        <Image src={ArrowIcon} alt="Arrow Icon" width={25} />
                    </Link>
                </div>
                <div className='border-y border-black mb-20'>
                    ...
                    <div className="flex justify-end pt-14 pb-10 px-11 font-light text-xl max-lg:text-lg max-lg:px-6 max-lg:pt-8">
